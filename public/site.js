(function(){
  ...
  if(hero){
    ...
    if(manifesto){
      ...
      function showPage(n){current=(n+pages.length)%pages.length;pages.forEach(function(p,i){p.hidden=i!==current});tabs.forEach(function(t,i){t.setAttribute('aria-selected',i===current?'true':'false')});status.textContent=(current+1)+' / '+pages.length;}
      ...
      var touchX=null;booklet.querySelector('.booklet-viewport').addEventListener('touchstart',function(e){touchX=e.changedTouches[0].clientX},{passive:true});booklet.querySelector('.booklet-viewport').addEventListener('touchend',function(e){if(touchX===null)return;var dx=e.changedTouches[0].clientX-touchX;if(Math.abs(dx)>40)showPage(current+(dx<0?1:-1));touchX=null},{passive:true});
    }
  }
