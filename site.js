(function(){
  ...
  if(hero){
    ...
    if(manifesto){
      ...
      + ' .mobile-booklet{display:block;padding:12px 14px 18px;background:#f0ebe5 url("paper-texture-tile.png") repeat;border-top:1px solid rgba(26,26,26,.12)}'
      + ' .booklet-tabs{display:grid;grid-template-columns:repeat(3,1fr);gap:10px;margin:0 0 12px}'
      ...
      + ' .booklet-viewport{position:relative;overflow:hidden;border:1px solid rgba(26,26,26,.14);background:#f4efe6;box-shadow:0 8px 24px rgba(26,26,26,.08)}'
      + ' .booklet-page{display:block;width:100%;height:auto}'
      + ' .booklet-page[hidden]{display:none}'
      + ' .booklet-controls{display:grid;grid-template-columns:1fr auto 1fr;align-items:center;gap:10px;margin-top:10px}'
      ...
      booklet.innerHTML=''
        ...
        + '  <button class="booklet-tab" role="tab" aria-selected="false" data-page="2">03</button>'
        + '</div>'
        + '<div class="booklet-viewport">'
        + '  <img class="booklet-page" src="graders_page1_clean.png" alt="The Graders manifesto page 1">'
        + '  <img class="booklet-page" src="graders_page2_final.png" alt="The Graders manifesto page 2" hidden>'
        + '  <img class="booklet-page" src="graders_page3_clean.png" alt="The Graders manifesto page 3" hidden>'
        + '</div>'
        + '<div class="booklet-controls">'
        + '  <button type="button" data-prev aria-label="Previous manifesto page">← Prev</button>'
        + '  <span class="booklet-status">1 / 3</span>'
